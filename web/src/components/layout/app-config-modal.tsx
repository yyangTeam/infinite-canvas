...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const visibleModelGroups = modelGroups.filter((g) => g.capability !== "video" || enableVideo);
    ...
    const addChannel = () => {
        const channel = createModelChannel({ name: t("config.channels.numberedName", { count: config.channels.length + 1 }), ...(fixedBaseUrl ? { baseUrl: fixedBaseUrl } : {}) });
                        ...
                        children: (
                            <Form layout="vertical" requiredMark={false}>
                                <section className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                                    ...
                                    {syncingWebdav || webdavSyncStatus ? <WebdavProgressGrid domainKeys={webdavDomainKeys} progress={webdavDomainProgress} t={t} /> : null}
                                </section>
                            </Form>
                        ),
                    },
...
function WebdavProgressGrid({ domainKeys, progress, t }: { domainKeys: AppSyncDomainKey[]; progress: Record<AppSyncDomainKey, WebdavDomainProgress>; t: TFunction }) {
    return (
